...
If you do not have xrandr, install a package called \textbf{autorandr}, it autodetects resolution and refresh rate and sets them. If you want to change them, just run the command mentioned above
...
\end{lstlisting}
If that does not exist either, just make it inside the .config/picom directory and copy the default setting from the official documentation.
Now that we have the configuration file ready to go, add it to the i3 config file:
\begin{lstlisting}
...
\centering
\includegraphics[width=0.60\textwidth]{i3final}
\caption{The assembled i3 configuration}
...
\subsection{Wayland with Sway}
Our pieces will be:
\begin{itemize}
    \item Sway as the window manager
    \item Waybar as the status bar
    \item Rofi-wayland as the application launcher
    \item Kitty as the terminal emulator
    \item Bash as the shell
    \item swaybg as the wallpaper manager
\end{itemize}

\subsubsection{Sway config}

After installing Sway and booting for the first time, we may encounter two problems: The keyboard layout, and the resolution.
Let's deal with the keys first. You will have to copy the config file from /etc/sway/config to ~/.config/sway/config. We do this with
\begin{lstlisting}
...
Now that out keys work, let's fix the resolution with:
\begin{lstlisting}
$ swaymsg get-outputs
\end{lstlisting}
